...
print(b) #输出为9
```
相当于在utf-8编码中，一个英文字符占用一个字节，一个中文字符占用三个字节
为了避免str和bytes两者转化之间出现乱码，所以坚持使用utf-8编码来对两者进行转换
...
>如果你不确定去替代的内容是什么类型的，直接用%s来替代也是可以的
```python
